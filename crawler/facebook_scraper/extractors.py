...
class PostExtractor:
    ...
    def extract_post(self) -> Post:
        ...
        for method in methods:
            try:
                ...
                sleep(600*random.uniform(1,1.5))
                raise
    ...
    def extract_comment_with_replies(self, comment):
        try:
            sleep(0.1*random.uniform(2,3))# sleep when comment get reply
            ...
            if replies_url:
                ...
                if replies:
                    replies = itertools.chain(result["replies"], reply_generator)
                else:
                    replies = reply_generator
            for reply in replies:
                result+=reply
            return result
        except exceptions.TemporarilyBanned:
    ...
    def extract_comments_full(self):
        ...
        while more_url and len(comments) <= limit:
            ...
            try:
                ...
                sleep(0.2*random.uniform(2,3)) #sleep when get more comment
            except exceptions.TemporarilyBanned:
                print("TemporarilyBanned")
                sleep(600*random.uniform(1,1.5))
                raise
